window.addEventListener("load",function() {
  ...
  Q.Sprite.extend("Flower", {
    init: function(p) {
      ...
      // Add 2d and animation modules
      this.add('2d, animation');

      this.on('bump.bottom', this,'kill');
      this.on('bump.left,bump.right', this, 'watered');
    } ,
    ...
    step: function(dt) { 
      ...
      if(this.p.x <= this.p.cx) {
        this.p.x = this.p.cx;
      } else if( this.p.x > Q.el.width - this.p.cx) {
        this.p.x = Q.el.width - this.p.cx;
      }
  ...
  Q.Sprite.extend("Gardener", {
    init: function(p) { 
      ...
      this.p.x = Math.round(Math.random()) == 0 ? -1 : (Q.el.width + this.p.w/2);
    ...
    step: function(dt) { 
      if( (this.p.x + this.p.cx < 0 && this.p.vx < 0) || (this.p.vx > 0 && this.p.x - this.p.cx > Q.el.width)) {
        this.destroy();
        return;
      }
